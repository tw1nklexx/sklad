...
const placeholder = `1 коробка - веник черный 20
1 коробка - ершик 12, совок 5
2 коробки - веник черный 4, совок 4`
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          showCloseButton
          className="max-h-[min(92vh,880px)] w-[min(100%-1rem,72rem)] gap-0 overflow-hidden border-border/60 p-0 shadow-xl sm:max-w-none"
        >
          ...
          <div className="grid max-h-[min(68vh,600px)] grid-cols-1 divide-y divide-border/60 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)] lg:divide-x lg:divide-y-0">
            <div className="flex min-h-0 flex-col gap-4 p-6">
              <div className="space-y-2">
                ...
                <Input
                  id="delivery-date"
                  type="date"
                  className="h-10 max-w-[11rem] rounded-lg border-border/80 bg-background"
                  disabled={isPending}
                  {...form.register("deliveryDate")}
                />
              </div>
              <div className="flex min-h-0 flex-1 flex-col gap-2">
                <Label htmlFor="shipment-text" className="text-muted-foreground">
                  Текст отгрузки
                </Label>
                <Textarea
                  id="shipment-text"
                  ...
                  rows={9}
                  className="min-h-[160px] flex-1 resize-none rounded-xl border-border/80 font-mono text-[13px] leading-relaxed"
                  disabled={isPending}
            ...
            <div className="flex min-h-0 flex-col gap-3 overflow-hidden p-6">
              <p className="text-sm font-medium text-foreground">Предпросмотр</p>
              {!preview ? (
                <div className="flex flex-1 items-center justify-center rounded-xl border border-dashed border-border/70 bg-muted/15 px-4 py-14 text-center text-sm text-muted-foreground">
                  Нажмите «Обработать»
                </div>
              ) : preview.length === 0 ? (
                <div className="flex flex-1 items-center justify-center rounded-xl border border-dashed border-border/70 bg-muted/15 px-4 py-14 text-center text-sm text-muted-foreground">
              ...
              {negative ? (
                <div
                  role="status"
                  className="rounded-xl border border-amber-200/90 bg-amber-50/90 px-3 py-2.5 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/25 dark:text-amber-50"
                >
          ...
          <DialogFooter className="flex-col gap-4 border-t border-border/60 bg-muted/25 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {actionError ? (
            <p
              className="border-t border-border/60 bg-destructive/5 px-6 py-3 text-sm text-destructive"
              role="alert"
            >
              {actionError}
            </p>
